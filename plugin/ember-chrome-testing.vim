if !exists("*s:YankTestId")
  function s:GetTestName()
    ...
    let test_name=strpart(base_url, test_line, test_name_start_y, test_name_len)
...
if !exists("*s:YankModuleName")
  function s:GetModuleName()
    ...
    let [_, x, y,_] = getpos('.') "get cursor position
    call cursor(x, 200) "move to 'emd of line' in case we're on a test line at the first char position
    ...
    let module_name=strpart(base_url, module_line, module_name_start_y, module_name_len)
